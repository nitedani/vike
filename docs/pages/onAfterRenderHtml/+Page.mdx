...
It isn't called if an earlier server hook sets <Link href="/pageContext#response">`pageContext.response`</Link> instead of rendering HTML.

It's called regardless of whether <Link href="/ssr">SSR</Link> is disabled. (For HTML output, the first page the user visits is always rendered to HTML: when SSR is disabled it's just an HTML shell that doesn't contain the content of the page.)

For <Link href="/pre-rendering">pre-rendered HTML pages</Link> it's called at build-time. (Since their HTML is generated at build-time.)
