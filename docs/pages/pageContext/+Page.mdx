...
An intentionally empty body stays empty: `new Response(null, { status: 204 })` doesn't receive an HTML fallback. `pageContext.response` isn't supported while <Link href="/pre-rendering">pre-rendering</Link>.
