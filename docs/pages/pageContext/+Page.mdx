...
The Web [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request) for the current server render, when one exists.

It is available when using <Link href="/server">`+server`</Link> and in Vike's development server. When calling <Link href="/renderPage">`renderPage()`</Link> yourself, it is `undefined` unless you provide `pageContextInit.request`.

Vike doesn't parse or consume the request body. A request body can be consumed only once: after a server hook reads it, subsequent hooks cannot read it again. When a request exists and middleware is configured, Vike clones it before dispatching it so that middleware cannot consume the hook's body.

<Prop name="response" />

A native [`Response`](https://developer.mozilla.org/en-US/docs/Web/API/Response) that a server hook can set to override the entire HTTP response, including its status, headers, and body.

An intentionally empty body stays empty: `new Response(null, { status: 204 })` doesn't receive an HTML fallback. `pageContext.response` isn't supported while <Link href="/pre-rendering">pre-rendering</Link>; set <Link href="#content" noWarning>`pageContext.content`</Link> from `onRenderHtml()` instead.

<Prop name="content" />

A string or [`ReadableStream`](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) set by the page-scoped <Link href="/onRenderHtml">`onRenderHtml()`</Link> hook. It becomes the HTTP response body when serving and the generated file's content when pre-rendering.

Set <Link href="#headersResponse" noWarning>`pageContext.headersResponse`</Link> to define headers such as `Content-Type`. Vike doesn't add a default HTML content type when `pageContext.content` is set.
...
The reason normal rendering was aborted. It is the caller-provided reason for <Link href="/render"><code>throw render()</code></Link>, `'Page Not Found'` when no route matches, or the thrown error when rendering fails. It is also used by the <Link href="/error-page">error page</Link>.
...
On the server-side, a new `pageContext` object is created for every render. The `pageContext` object is discarded after the HTTP response is sent to the client. (But `pageContext` properties sent to the client via <Link href="/passToClient">passToClient</Link> are preserved on the client-side.)
...
Upon <Link href="/pre-rendering">pre-rendering an HTML page</Link>, the `pageContext` object used for pre-rendering is preserved and saved twice at:
